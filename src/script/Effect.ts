...
abstract class Effect {
  ...
  abstract async onLot(
    lot: LotBase | null,
    control: ControlName,
    gameMode: GameMode,
    bonusFlag: BonusFlag
  ): Promise<void>;
  abstract async payEffect(
    payCoin: number,
    hitYakus: HitYakuData[],
    gameMode: GameMode,
    bonusFlag: BonusFlag
  ): Promise<void>;
  async onPay(
  ...
  abstract async onBonusEnd(bonusData: Bonus): Promise<void>;
}
...
export class EffectManager {
  ...
  Segments = {
    PaySeg: segInit(
      document.querySelector("#paySegment") as HTMLCanvasElement,
      2
    ),
    EffectSeg: segInit(
      document.querySelector("#effectSegment") as HTMLCanvasElement,
      3
    ),
    CreditSeg: segInit(
      document.querySelector("#creditSegment") as HTMLCanvasElement,
      2
    ),
  };
...
class NormalEffect extends Effect {
  ...
  ARTBitaTable = {
    GameCount: [3, 5, 10, 30, 50, 103, 105, 110],
    LotValue: [900, 50, 41, 3, 3, 1, 1, 1],
  };
  nabiElements = Array.from(document.querySelectorAll("#nabi1,#nabi2,#nabi3"));
  stopLightElements = Array.from(
    document.querySelectorAll("#stop1,#stop2,#stop3")
  );
  startCoin: number = 0;
  isHyper = false;
  ...
  async onLot(
    ...
    if (this.ExtraART) {
      this.effectManager.Segments.EffectSeg.setSegments(
        `-${this.ExtraARTGame}-`
      );
    } else {
    }
    ...
    switch (gameMode) {
      case "Normal":
        ...
        switch (control) {
          ...
          case ControlName["3択子役3"]:
            if (this.isART || this.ExtraART || !Rand(64)) {
              if (!this.isART || this.ARTStock > 0 || this.ExtraART) {
                ...
                this.changeARTSeg();
                if (!this.ExtraART) this.ARTStock--;
              }
              Slot.freeze();
          ...
          case ControlName["REG"]:
            ...
            if (
              (this.stop3Flag && !Rand(4)) ||
              (bonusFlag && this.stop3Flag && !Rand(3))
            ) {
              this.stop3Flag = false;
              spStopStack = [];
              Slot.freeze();
            ...
            if (bgmStopFlag) {
              Slot.once("allReelStop", () => {
                if (gameMode == "Normal") sounder.stopSound("BGM");
              });
            }
        }
        Slot.once("bet", () => {
      ...
      case "BIG":
        ...
        let segs = this.effectManager.Segments.EffectSeg.randomSeg().slice(
          0,
          3
        );
        ...
        let bitaMiss = false;
        console.log(Slot.reelController.reels[1].reelChipPosition);
        if (
          event.data.idx !== 1 ||
          ![0, 1].includes(Slot.reelController.reels[1].reelChipPosition)
        ) {
          ...
          let shuffle = () => {
            setTimeout(() => {
              ...
              for (let i = 2; i >= stopCount; i--) {
                this.effectManager.Segments.EffectSeg.setSegment(
                  i + 1,
                  "" + Rand(10)
                );
              }
  ...
  async onBonusEnd(bonusData: Bonus) {
    ...
    if (bonusData.name.includes("BIG")) {
      ...
      if (!Rand(3)) {
        ...
        let r = [0.5, 0.75, 0.88, 1.0][ArrayLot([25, 30, 30, 15])];
        this.ExtraART = true;
        this.ExtraARTLot = r;
        this.ExtraARTGame = NormalEffect.ExtraARTGameBase;
  ...
  async WaveSeg(level: number, wait: number = 50) {
    ...
    for (let [idx, st] of PulseTable[level].entries()) {
      if (!st) return;
      for (let s of st) {
        await this.effectManager.Segments.EffectSeg.segments[idx].draw({
          [s]: 1,
        });
        await Sleep(wait);
