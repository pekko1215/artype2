...
export class SlotReelController {
    ...
    getHitYakus(): HitYakuData[] {
        ...
        betLine.forEach((line, idx) => {
            ...
            yakuList.forEach((d, j) => {
                ...
                hitYaku.push(new class implements HitYakuData {
                    line = idx;
                    matrix = matrix;
                    index = j;
                });
    ...
    getMoveingCount(): number {
        return this.reels.filter(reel => reel.status === ReelStatus.Move || reel.status === ReelStatus.Sliping).length
    }
    stopReel(idx: number): boolean {
        ...
        if (this.reels.some(reel => [ReelStatus.Sliping].includes(reel.status))) return false;
    ...
    reelSlipStart(reelIndex: number, slips: number) {
        ...
        if (reel.slipLength < 0) {
            reel.slipLength += this.slotModule.viewController.reelChipData!.height * slips;
        }
    }
